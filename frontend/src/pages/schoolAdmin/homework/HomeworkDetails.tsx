...
const labelOf = (value: any) => {
  ...
  return value?.name ?? value?.sessionName ?? "—";
};
...
const HomeworkDetails: React.FC = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useSelector((state: any) => state.homework ?? {});
  ...
  const stats = useSelector(
    (state: any) => state.homeworkSubmission?.stats ?? null,
  );
  ...
  useEffect(() => {
    if (
      ...
      !selectedSessionId ||
      h?._id !== homeworkId ||
      getId(h.sessionId) !== selectedSessionId
    ) {
      return;
    }

    dispatch(getHomeworkSubmissionStats(homeworkId));
  }, [dispatch, homeworkId, h?._id, h?.sessionId, selectedSessionId]);

  // ====================================================
  // CHANGE STATUS
  // ====================================================

  const handleStatusChange = async (status: "PUBLISHED" | "CLOSED") => {
    if (
      !h?._id ||
      !selectedSessionId ||
      getId(h.sessionId) !== selectedSessionId
    ...
    try {
      ...
      console.error("Failed to change homework status:", err);
    }
  };
  ...
  if (getId(h.sessionId) !== selectedSessionId) {
    return (
      <div className="min-h-screen bg-slate-50 p-8">
        <HomeworkState
          type="error"
  ...
  const submitted = stats?.totalSubmitted ?? 0;
  ...
  const total = stats?.totalStudents ?? submitted;

  const pending = stats?.pendingStudents ?? Math.max(total - submitted, 0);
  ...
  let displayStatus: "Draft" | "Active" | "Overdue" | "Completed";

  if (h.status === "DRAFT") {
    displayStatus = "Draft";
  } else if (h.status === "CLOSED") {
    displayStatus = "Completed";
  } else if (new Date(h.dueDate).getTime() < Date.now()) {
    ...
    <div className="min-h-screen bg-slate-50">
      <main className="mx-auto w-full max-w-[1240px] space-y-6 p-6 lg:p-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {h.status === "DRAFT" && (
              <>
                <button
                  onClick={() =>
                    navigate(`/school-admin/homework/${h._id}/edit`)
                  }
            ...
            {h.status === "PUBLISHED" && (
              <>
                ...
                <button
                  onClick={() =>
                    navigate(`/school-admin/homework/${h._id}/submissions`)
                  }
        ...
        <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[
            ["Total Students", total, "text-slate-900"],

            ["Submitted", submitted, "text-emerald-600"],

            ["Pending", pending, "text-slate-900"],

            ["Late", late, "text-red-600"],
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      {h.attachment.fileType ?? "Attachment"}{" "}
            ...
            <div className="mt-6 border-t border-slate-200 pt-6">
              ...
              <div className="mt-2 flex justify-between text-xs text-slate-500">
                ...
                <span>{stats?.pendingReview ?? 0} pending review</span>
              </div>
            </div>
          </section>
          ...
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <dl className="mt-5 space-y-5 text-sm">
              {[
                ["Academic Year", labelOf(h.sessionId)],

                ["Class", labelOf(h.classId)],

                ["Section", labelOf(h.sectionId)],

                ["Subject", labelOf(h.subjectId)],

                ["Teacher", labelOf(h.teacherId)],
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                onClick={() =>
                  handleStatusChange(
                    confirmAction === "CLOSE" ? "CLOSED" : "PUBLISHED",
                  )
                }
