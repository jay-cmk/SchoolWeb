...
interface ClassWiseStudentCount {
  classId: string;
  className: string;
...
interface StudentStatsProps {
  ...
  // Temporary compatibility with the current Dashboard.tsx.
  // These can be removed after Dashboard.tsx is updated.
  selectedYear?: string;
  onYearChange?: (year: string) => void;
}
...
const StudentStats: React.FC<StudentStatsProps> = ({
  ...
  const data = useMemo(
    () => ({
      labels: classWiseStudents.map(
        (item) => item.className,
      ),
      datasets: [
        {
          label: "Students",
          data: classWiseStudents.map(
  ...
  const malePercentage =
    genderTotal > 0
      ? (maleStudents / genderTotal) * 100
      : 0;

  const femalePercentage =
    genderTotal > 0
      ? (femaleStudents / genderTotal) * 100
      : 0;
  ...
  return (
    <article className="rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm xl:col-span-2">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-lg font-semibold text-[#15243B]">
            Student statistics
          </h2>
          <p className="mt-1 text-sm text-[#6B7280]">
            Class-wise distribution across Classes 1–12
          </p>
        </div>

        <div className="flex min-h-11 items-center rounded-lg border border-[#E5E7EB] px-3 text-xs font-semibold text-[#15243B]">
          {displaySessionName}
          <Icon
            icon="lucide:calendar-days"
            className="ml-1 text-[#1F5FAE]"
          />
        </div>
      </div>

      <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="flex flex-col justify-between rounded-lg bg-[#F9FAFB] p-4">
          <div>
            <p className="text-xs font-medium text-[#6B7280]">
              Total students
            </p>
            <p className="mt-1 text-2xl font-bold text-[#15243B]">
          ...
          <div>
            <div className="mt-4 flex items-center justify-between text-xs">
              <span className="flex items-center gap-1 text-[#6B7280]">
                <i className="inline-block h-2 w-2 rounded-full bg-[#1F5FAE]" />
                Male {maleStudents.toLocaleString("en-IN")}
              </span>
              <span className="flex items-center gap-1 text-[#6B7280]">
                <i className="inline-block h-2 w-2 rounded-full bg-[#10B981]" />
                Female {femaleStudents.toLocaleString("en-IN")}
              </span>
            </div>
        ...
        <div className="lg:col-span-2">
          <div className="h-64">
            {classWiseStudents.length > 0 ? (
              ...
              <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-[#D1D5DB] text-sm text-[#6B7280]">
