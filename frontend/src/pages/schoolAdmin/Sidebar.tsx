...
const MenuGroup = ({
  ...
  const groupActive = currentPath.startsWith(basePath);
  ...
  return (
    <div>
      <button
        ...
        className={`flex min-h-11 w-full items-center gap-3 rounded-lg px-3 font-semibold transition-all duration-200 ${
          groupActive || open
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-10 w-full items-center gap-3 rounded-lg px-3 text-sm font-medium transition-all duration-200 ${
                  itemActive
                    ? "bg-[#E8F0FB] text-[#1F5FAE]"
                    : "text-[#6B7280] hover:bg-[#F9FAFB] hover:text-[#15243B]"
                }`}
...
const academicItems: MenuItem[] = [
  ...
  { id: "assignment", label: "Assignment", icon: "lucide:graduation-cap", path: "/school-admin/academic/subject-assignments" },
];

const studentItems: MenuItem[] = [
  { id: "all-students", label: "All Students", icon: "lucide:users", path: "/school-admin/students" },
  { id: "add-student", label: "Add Student", icon: "lucide:user-plus", path: "/school-admin/students/add" },
  { id: "class-wise-students", label: "Class-wise Students", icon: "lucide:school", path: "/school-admin/students/class-wise" },
  { id: "bulk-promotion", label: "Bulk Promotion", icon: "lucide:arrow-up-right", path: "/school-admin/students/bulk-promotion" },
];

const teacherItems: MenuItem[] = [
  { id: "all-teachers", label: "Teachers", icon: "lucide:users", path: "/school-admin/teachers" },
  { id: "mark-teacher-attendance", label: "Mark Attendance", icon: "lucide:user-check", path: "/school-admin/teacher-attendance/mark" },
  { id: "daily-teacher-attendance", label: "Daily Attendance", icon: "lucide:calendar-days", path: "/school-admin/teacher-attendance/daily" },
  { id: "monthly-teacher-attendance", label: "Monthly Summary", icon: "lucide:calendar-range", path: "/school-admin/teacher-attendance/monthly" },
];

const studentAttendanceItems: MenuItem[] = [
  { id: "mark-attendance", label: "Mark Attendance", icon: "lucide:user-check", path: "/school-admin/attendance/mark" },
  { id: "daily-attendance", label: "Daily Attendance", icon: "lucide:calendar-days", path: "/school-admin/attendance/daily" },
  { id: "monthly-attendance", label: "Monthly Attendance", icon: "lucide:calendar-range", path: "/school-admin/attendance/monthly" },
];

const timetableItems: MenuItem[] = [
  { id: "weekly-timetable", label: "Weekly Timetable", icon: "lucide:calendar-range", path: "/school-admin/timetable/weekly" },
  { id: "daily-timetable", label: "Daily Timetable", icon: "lucide:calendar-days", path: "/school-admin/timetable/daily" },
  { id: "teacher-timetable", label: "Teacher Timetable", icon: "lucide:user-round", path: "/school-admin/timetable/teacher" },
];
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/school-admin/academic")) setIsAcademicOpen(true);
    if (path.startsWith("/school-admin/students")) setIsStudentsOpen(true);
    if (
      path.startsWith("/school-admin/teachers") ||
      path.startsWith("/school-admin/teacher-attendance")
    ) {
      setIsTeachersOpen(true);
    }
    if (path.startsWith("/school-admin/attendance")) setIsStudentAttendanceOpen(true);
    if (path.startsWith("/school-admin/timetable")) setIsTimetableOpen(true);
    ...
    if (matchedItem && currentTab !== matchedItem.id) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-[#E5E7EB] bg-white transition-all duration-300 ease-in-out lg:relative lg:h-full lg:translate-x-0 ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex shrink-0 items-center justify-between border-b border-[#E5E7EB] px-5 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7280]">School ERP</p>
            </div>
          </div>

          <button type="button" onClick={onClose} className="p-1 text-[#6B7280] hover:text-[#15243B] lg:hidden">
            <Icon icon="lucide:x" className="text-xl" />
          </button>
        </div>

        <nav className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 [scrollbar-width:thin]">
          <p className="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-[#6B7280]">
            Operations
          </p>
          ...
          <div className="space-y-1 text-sm">
            ...
            <MenuGroup label="Academic" icon="lucide:book-open" basePath="/school-admin/academic" items={academicItems} open={isAcademicOpen} setOpen={setIsAcademicOpen} currentPath={location.pathname} onNavigate={handleNavigation} />
            <MenuGroup label="Students" icon="lucide:users" basePath="/school-admin/students" items={studentItems} open={isStudentsOpen} setOpen={setIsStudentsOpen} currentPath={location.pathname} onNavigate={handleNavigation} />

            <MenuGroup
              label="Teacher "
              icon="lucide:clipboard-check"
              basePath="/school-admin/teacher"
              items={teacherItems}
              open={isTeachersOpen}
            ...
            />
            <MenuGroup label="Student Attendance" icon="lucide:calendar-check" basePath="/school-admin/attendance" items={studentAttendanceItems} open={isStudentAttendanceOpen} setOpen={setIsStudentAttendanceOpen} currentPath={location.pathname} onNavigate={handleNavigation} />
            <MenuGroup label="Timetable" icon="lucide:calendar-clock" basePath="/school-admin/timetable" items={timetableItems} open={isTimetableOpen} setOpen={setIsTimetableOpen} currentPath={location.pathname} onNavigate={handleNavigation} />

            {otherItems.map(renderSingleItem)}
          </div>
        </nav>

        <div className="shrink-0 space-y-1 border-t border-[#E5E7EB] p-4 text-sm">
          <button type="button" onClick={() => handleNavigation("help", "/help")} className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-[#6B7280] transition-colors hover:bg-[#F9FAFB] hover:text-[#15243B]">
            <Icon icon="lucide:circle-help" className="text-lg" />
            Help & Support
          </button>

          <button type="button" onClick={() => handleNavigation("profile", "/profile")} className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-[#6B7280] transition-colors hover:bg-[#F9FAFB] hover:text-[#15243B]">
          ...
          <button
            ...
            className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-[#EF4444] transition-colors hover:bg-[#FEF2F2]"
          >
