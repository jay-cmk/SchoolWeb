    ...
    <div className="min-h-full bg-[#F7F9FC] p-4 md:p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white">
          ...
          <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ["Academic Session", relationName(selectedStudent?.sessionId)],
              ["Class", relationName(selectedStudent?.classId)],
              ["Section", relationName(selectedStudent?.sectionId)],
              ["Roll Number", selectedStudent?.rollNumber?.toString() ?? "—"],
            ].map(([label, value]) => (
              <div
                key={label}
                className="rounded-lg border border-[#E5E7EB] bg-[#F9FAFB] px-4 py-3"
              >
                ...
                <p className="mt-1 font-semibold text-[#15243B]">{value}</p>
              </div>
            ))}
          </div>
        </div>
