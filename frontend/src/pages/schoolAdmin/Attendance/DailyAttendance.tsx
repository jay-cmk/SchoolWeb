...
const getStudentInfo = (attendance: AttendanceData) => {
  ...
  return {
    ...
    firstName: attendance.studentId.firstName,

    lastName: attendance.studentId.lastName || "",

    admissionNumber: attendance.studentId.admissionNumber || "-",

    rollNumber: attendance.studentId.rollNumber ?? "-",

    profileImage: attendance.studentId.profileImage || "",
  };
};
...
const DailyAttendance: React.FC = () => {
  ...
  const availableClasses = useMemo(() => {
    ...
    return classes.filter(
      (classData) =>
        getRelationId(classData.sessionId) === selectedSessionId &&
  ...
  useEffect(() => {
    ...
    const selectedExists = availableClasses.some(
  ...
  const availableSections = useMemo(() => {
    ...
    return sections.filter((section) => {
      const sessionId = getRelationId(section.sessionId);

      const classId = getRelationId(section.classId);
  ...
  useEffect(() => {
    ...
    const selectedExists = availableSections.some(
  ...
  const handleMarkAttendance = () => {
    ...
    navigate(`/school-admin/attendance/mark?${getCurrentQuery()}`);
  };
  ...
  const handleEditRecord = () => {
    ...
    navigate(`/school-admin/attendance/update?${getCurrentQuery()}`);
  };
  ...
  const handleMonthlyView = () => {
    ...
    navigate(`/school-admin/attendance/monthly?${params.toString()}`);
  };
  ...
  const handleStudentClick = (record: AttendanceData) => {
    const student = getStudentInfo(record);
    ...
    <div className="min-h-screen bg-gray-50 p-5 lg:p-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Review finalized attendance records by class and section for the
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="flex min-h-11 items-center gap-2 rounded-lg bg-blue-600 px-5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex min-h-11 items-center gap-2 rounded-lg border border-gray-300 bg-white px-5 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Icon icon="lucide:calendar-range" />
            Monthly View
          </button>
        ...
        <div className="mt-5 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-amber-700">
              Attendance, classes and sections are session-wise.
            </p>
          </div>
        </div>
      )}
      ...
      <section className="mt-5 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-200 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900">
              {selectedClass?.name || "Class"} · Section{" "}
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Select an academic session from the topbar, then choose class and section."
          />
          ...
          <EmptyState
            ...
            } - Section ${selectedSection?.name || ""} on ${formatDate(
              selectedDate,
            )}.`}
            ...
            <table className="w-full min-w-[1050px] text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredAttendance.map((record) => {
                  ...
                  const className =
                    getRelationName(record.classId) ||
                    selectedClass?.name ||
                    "-";

                  const sectionName =
                    getRelationName(record.sectionId) ||
                    selectedSection?.name ||
                    "-";
