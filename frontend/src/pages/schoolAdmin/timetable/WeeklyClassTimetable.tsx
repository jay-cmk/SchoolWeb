...
// const currentTimetableDay = (): string => {
...
// const WeeklyClassTimetable: React.FC = () => {
...
//   const { timetable, loading, error, copyResult } = useAppSelector(
...
//       (item) =>
//         getRelationId(item.sessionId) === sessionId &&
//         getRelationId(item.classId) === targetClassId &&
//         item.isActive !== false,
//     );
//   }, [sections, sessionId, targetClassId]);
...
//       replace: true,
//     });
//   }, [sessionId, classId, sectionId, teacherId, setSearchParams]);
...
//     dispatch(getTimetable(filters));
//   };

//   useEffect(() => {
//     fetchTimetable();
//   }, [dispatch, sessionId, classId, sectionId, teacherId]);
...
//       const existing = map.get(item.periodNumber);

//       if (!existing || item.startTime < existing.startTime) {
//         map.set(item.periodNumber, {
...
//   ): Timetable | undefined =>
//     timetable.find(
//       (item) => item.day === day && item.periodNumber === periodNumber,
//     );

//   const todayPeriods = useMemo(() => {
//     const day = currentTimetableDay();

//     if (!day) {
//       return 0;
//     }

//     return timetable.filter((item) => item.day === day).length;
//   }, [timetable]);

//   const totalActiveClasses = useMemo(
//     () =>
//       new Set(
//         timetable.map((item) => getRelationId(item.classId)).filter(Boolean),
//       ).size,
//     [timetable],
//   );
...
//     () =>
//       new Set(
//         timetable.map((item) => getRelationId(item.sectionId)).filter(Boolean),
//       ).size,
//     [timetable],
//   );
...
//       timetable.filter(
//         (item) =>
//           item.periodType === TimetablePeriodType.REGULAR &&
//           (!item.subjectId || !item.teacherId),
//       ).length,
//     [timetable],
//   );

//   const openAdd = (context: AddContext = {}) => {
...
//     const payload: CopyTimetableData = {
//       sessionId,
...
//     const result = await dispatch(copyTimetable(payload));

//     if (copyTimetable.fulfilled.match(result)) {
//       setCopyOpen(false);

//       if (result.payload.conflicts.length > 0) {
//         setConflictMessage(result.payload.conflicts.join("\n"));
//       }
//     }
//   };

//   const periodCardClass = (type: string) => {
//     if (
//       type === TimetablePeriodType.BREAK ||
//       type === TimetablePeriodType.LUNCH
//     ) {
//       return "border-sky-200 bg-sky-50";
//     }

//     if (type === TimetablePeriodType.ACTIVITY) {
...
//             <h1 className="mt-2 text-3xl font-bold text-gray-950">Timetable</h1>

//             <p className="mt-1 text-sm text-gray-500">
//               Create and manage class, section and teacher schedules for the
...
//             type="button"
//             onClick={() => openAdd()}
//             disabled={!sessionId || !classId || !sectionId}
...
//                 Classes, sections and timetable data use the academic session
...
//           <div className="mb-4">
//             <h2 className="font-bold text-gray-900">Schedule filters</h2>

//             <p className="mt-1 text-xs text-gray-500">
//               Academic Session comes from the topbar. Select Class → Section to
//               load the timetable.
//             </p>
//           </div>
...
//               <Icon icon="lucide:triangle-alert" className="mt-0.5 text-lg" />
...
//                 Copy completed: {copyResult.copied} copied, {copyResult.skipped}{" "}
//                 skipped.
//               </p>

//               <button type="button" onClick={() => dispatch(clearCopyResult())}>
//                 <Icon icon="lucide:x" />
//               </button>
//             </div>
//           </div>
...
//                   navigate(
//                     `/school-admin/timetable/teacher?${params.toString()}`,
//                   );
//                 }}
//               />
...
//               description="Select an academic session from the topbar, then choose class and section to load its weekly timetable."
//             />
//           ) : loading ? (
//             <LoadingState />
//           ) : timetable.length === 0 ? (
...
//                   <tr>
//                     <th className="w-36 px-4 py-3 text-left">Time / Period</th>

//                     {DAYS.map((day) => (
//                       <th key={day} className="px-2 py-3 text-left">
//                         {DAY_LABEL[day]}
//                       </th>
//                     ))}
//                   </tr>
...
//                   {slots.map((slot) => (
//                     <tr key={slot.periodNumber} className="align-top">
...
//                       {DAYS.map((day) => {
//                         const period = findPeriod(day, slot.periodNumber);
...
//                                 {getRelationName(period.teacherId) ||
//                                   "No teacher"}
//                               </p>

//                               <p className="mt-1 text-xs text-gray-500">
//                                 {period.roomNumber || "No room"}
//                               </p>
//                             </button>
//                           </td>
//                         );
...
//           sectionId={sectionId}
//           day={addContext.day}
//           periodNumber={addContext.periodNumber}
//           startTime={addContext.startTime}
//           endTime={addContext.endTime}
...
//                 Source: <strong>{selectedClass?.name || "-"}</strong>
//                 {" · Section "}
...
//                 options={targetSections.map((item) => ({
//                   value: item._id,
...
const WeeklyClassTimetable: React.FC = () => {
  ...
  }, [dispatch, selectedSessionId, sessionId]);
  ...
  }, [classId, sectionId, teacherId, setSearchParams]);
          ...
          <div className="flex flex-col gap-4 border-b border-gray-200 p-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <ActionButton
                icon="lucide:user-round"
                label="Teacher Timetable"
                onClick={() => navigate("/school-admin/timetable/teacher")}
              />
