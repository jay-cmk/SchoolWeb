...
//       select: "teacherId weeklyPeriods assignmentType stream isActive",
...
import { sendMobilePushSafely } from "../notifications/pushNotification.service";
...
/* =====================================================
   NOTIFY STUDENT ABOUT ELECTIVE ASSIGNMENT

   Student के साथ linked User account होना आवश्यक है.

   Notification या push fail होने पर elective assignment
   operation fail नहीं होगा.
===================================================== */

const notifyStudentElectiveAssigned = async (
  schoolId: string,
  studentId: string,
  studentSubjectEnrollmentId: string,
  notificationTitle: string = "Elective subject assigned",
): Promise<void> => {
  try {
    const enrollment = await StudentSubjectEnrollment.findOne({
      _id: studentSubjectEnrollmentId,
    ...
    const [student, subject, classData, section, session] = await Promise.all([
      Student.findOne({
        _id: studentId,
    ...
    /*
     * Student का login User account link नहीं है
     * तो notification receive नहीं कर सकता.
     */
    if (!student?.userId || !subject || !classData || !section) {
      return;
    }
    ...
    const notification = await createNotification({
      ...
      type: "ELECTIVE_ASSIGNED",

      title: notificationTitle,

      message:
        `${subject.name} has been added to your subjects for ` +
      ...
      metadata: {
        ...
        studentSubjectEnrollmentId: enrollment._id.toString(),

        assignmentId: enrollment.subjectAssignmentId.toString(),
        ...
        sessionId: enrollment.sessionId.toString(),
     ...
     * Mobile push background में भेजी जाएगी.
     */
    void sendMobilePushSafely(notification);
  } catch (error) {
    console.error("STUDENT ELECTIVE NOTIFICATION ERROR:", error);
  }
};
...
export const assignStudentElectiveSubject = async (
  ...
  const student = await Student.findOne({
    ...
    .select("_id name admissionNumber status userId")
    .lean();
  ...
  /* ===============================================
       ...
       Student और subject assignment का session,
       class और section same होना चाहिए.
    =============================================== */
  ...
  /* ===============================================
       ...
       CLASS:
       Same placement के सभी streams के students.

       STREAM:
       केवल matching stream के students.
    =============================================== */
  ...
  if (existingEnrollment?.status === "DROPPED") {
    ...
    await notifyStudentElectiveAssigned(
      schoolId,
  ...
  const populatedEnrollment = await getPopulatedStudentSubject(
    ...
    electiveEnrollment._id.toString(),
  );
...
/* =====================================================
   ...
   assignStudentElectiveSubject प्रत्येक successful
   student के लिए notification भी बनाएगा.
===================================================== */
...
export const bulkAssignStudentElectiveSubject = async (
  ...
  for (const studentId of uniqueStudentIds) {
    try {
      const enrollment = await assignStudentElectiveSubject(
        schoolId,
...
export const updateStudentElectiveSubject = async (
  ...
  /*
   * Status/remarks update पर controlled scope में
