...
const buildStudentBody = (
  ...
  /*
   * Client se aaya raw photo value trust nahi karna.
   * Multer-uploaded file hi string path banegi.
   */
  delete body.photo;
  ...
  if (
    ...
    if (rollNumber) {
      body.rollNumber = Number(rollNumber);
    } else {
  ...
  if (
    typeof body.penNumber === "string"
  ) {
    const penNumber =
      body.penNumber.replace(/\s/g, "");

    if (penNumber) {
      body.penNumber = penNumber;
    } else {
      delete body.penNumber;
    }
  }


  if (
    typeof body.apaarId === "string"
  ) {
    const apaarId =
      body.apaarId.replace(/\s/g, "");

    if (apaarId) {
      body.apaarId = apaarId;
    } else {
      delete body.apaarId;
    }
  }
