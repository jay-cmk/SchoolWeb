...
export class StudentService {
  ...
  private validatePenNumber(penNumber: string) {
    const value = penNumber.replace(/\s/g, "").trim();

    if (!/^\d{11}$/.test(value)) {
      throw new Error("PEN Number must be exactly 11 digits");
    }
  ...
  private normalizeAddress(
    ...
    const normalizedAddress: IStudentAddress = {};

    const addTrimmedValue = (
      key: keyof IStudentAddress,
      value?: string,
      uppercase = false,
    ) => {
      ...
      normalizedAddress[key] = uppercase
        ? trimmedValue.toUpperCase()
        : trimmedValue;
    };

    addTrimmedValue("addressLine", address.addressLine);
    addTrimmedValue("city", address.city);
    addTrimmedValue("district", address.district);
    addTrimmedValue("districtCode", address.districtCode);
    addTrimmedValue("state", address.state);
    addTrimmedValue("stateCode", address.stateCode);
    addTrimmedValue("pincode", address.pincode);
    addTrimmedValue("country", address.country);
    addTrimmedValue("countryCode", address.countryCode, true);

    return normalizedAddress;
  }

  // ============================================
  // VALIDATE STREAM FOR CLASS 11 / 12
  // ============================================

  private validateStreamForClass(
    classData: {
      name?: string;
      order?: number;
    },
    ...
    const normalizedStream = stream
      ?.trim()
      .toUpperCase() as StudentStream | undefined;

    if (
      normalizedStream &&
      !allowedStreams.includes(normalizedStream)
    ) {
      throw new Error(
    ...
    if (isSeniorSecondaryClass && !normalizedStream) {
      throw new Error("Stream is required for Class 11 and Class 12");
    }

    if (!isSeniorSecondaryClass && normalizedStream) {
      throw new Error(
        "Stream can only be selected for Class 11 or Class 12",
      );
    }
  ...
  async createStudent(
    ...
    const stream = this.validateStreamForClass(
      classData,
      data.stream,
    );
    ...
    // ==========================================
    // DUPLICATE PEN NUMBER
    // ==========================================

    if (data.penNumber) {
      const penNumber = this.validatePenNumber(data.penNumber);

      const existingPenNumber = await Student.findOne({
        schoolId,
        penNumber,
      }).lean();

      if (existingPenNumber) {
        throw new Error("PEN Number already exists");
      }
    }
    ...
    if (data.penNumber) {
      createData.penNumber = this.validatePenNumber(data.penNumber);
    }
  ...
  createData.photo =
    ...
    if (data.address) {
      createData.address = this.normalizeAddress(data.address);
    }
    ...
    if (data.currentAddress) {
      createData.currentAddress = this.normalizeAddress(
        data.currentAddress,
      );
    }
    ...
    if (data.permanentAddress) {
      createData.permanentAddress = this.normalizeAddress(
        data.permanentAddress,
      );
    }
  ...
  async updateStudent(
    ...
    if (data.penNumber !== undefined && data.penNumber.trim()) {
      const penNumber = this.validatePenNumber(data.penNumber);

      const duplicatePenNumber = await Student.findOne({
        _id: {
          $ne: studentId,
        },
    ...
    if (data.penNumber !== undefined) {
      updateData.penNumber = data.penNumber.trim()
        ? this.validatePenNumber(data.penNumber)
        : undefined;
    }
 ...
 if (
  ...
  updateData.photo =
    ...
    if (data.address !== undefined) {
      updateData.address = this.normalizeAddress(data.address);
    }
    ...
    if (data.currentAddress !== undefined) {
      updateData.currentAddress = this.normalizeAddress(
        data.currentAddress,
      );
    }
    ...
    if (data.permanentAddress !== undefined) {
      updateData.permanentAddress = this.normalizeAddress(
        data.permanentAddress,
      );
    }
